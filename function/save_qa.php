	...
	if (mysql_query("BEGIN",$conn)) {
			...
			if($qa_id == 0){
				$sql = " INSERT ignore INTO t_qa
					 (  name,phone,email,cus_type,con_type,responsi,title,content,create_date,update_date,status) ";
				$sql .= " VALUES ";
				$sql .= "( $name,$phone,$email,$cus_type,$con_type,$respons,$title,$content,now(),now(),1 )";
