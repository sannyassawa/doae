<?php
	...
	if (mysql_query("BEGIN",$conn)) {
			...
			$sql = " SELECT MAX(about_id) AS mns";
			$sql .= " FROM t_about  ";
			...
			$sql = " INSERT ignore INTO t_about
					 ( create_id,parent_id, create_date, update_id,update_date,sort_order,active) ";
			$sql .= " VALUES ";
			$sql .= "( $userid,$parent_id, now(), $userid,now(),$sort_order,1 )";
			...
			$sql = " SELECT MAX(about_id) AS MaxID ";
			$sql .= " FROM t_about  ";
