		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
						...
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_article_sub.php?article_id=$article_id");
		            }
		        }
		    }
...
<div class="container">
		<div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">จัดการหมวดหมู่ บทความการเกษตร</h3>
				<ol class="breadcrumb">
								...
								<div class="form-group">
                                    ...
                                    <select class="form-control" name="cat_article_id" id="cat_article_id" >
										<?php
											...
											$sql2 = "SELECT * FROM t_article_cat
													WHERE cat_article_id = ".$row[cat_article_id];
													$objQuery2 = mysql_query($sql2);
													$row2 = mysql_fetch_array($objQuery2);
														echo "<option value='".$row2['cat_article_id']."'>".$row2['name']."</option>";
													
											
											$sql1 = "SELECT * FROM t_article_cat
													WHERE active = 1";
											$objQuery1 = mysql_query($sql1);
											while ($row1 = mysql_fetch_array($objQuery1)) {
												
											
											echo "<option value='".$row1['cat_article_id']."'>".$row1['name']."</option>";
...
function cmdCancel(){
	...
	window.location.href = "article_sub.php?Cat=" + cate;
...
function cmdSave(){
	...
	$.ajax({
		type: "POST",
		url: "function/save_sub_article.php",
		data: params,
