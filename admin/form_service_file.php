...
$userid = $_SESSION['userid'];
	...
	$sql = "select * from t_service_file
			where file_id = ".$_GET['service_id'];
		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
						...
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_service_file.php?service_id=$file_id");
		            }
		        }
		    }
                        ...
                        <div class="panel-body">
						   ...
						   <input type="hidden" name="user_id" id="user_id" value="<?=$_SESSION['userid'] ?>">
								<div class="form-group">
                                    ...
                                    <p><img src="<?=$row['path'] ?>" height="100" width="200" /></p>
									...
									<p class="help-block">โปรดระบุ ขนาดรูปภาพ : 200 x 100px</p>
                                </div>
								<div class="form-group">
                                    <label>url</label>
