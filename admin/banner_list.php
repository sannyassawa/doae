		...
		<div class="row">
			<?php
				$sql = "SELECT * FROM t_banner";
						...
						$sql .= " ORDER BY CONVERT(sort_order , UNSIGNED INTEGER) DESC ";
					...
					echo "<div  class='adminDiv text-right'> ";
                        ...
                        if($i==1){
                            ...
                            echo "<a href='' onclick='upBanner(".$row['sort_order'].")' class='btn btn-default btn-xs'><i class='fa fa-arrow-up'></i></a>";}
                           # echo '<a href=# onclick="return ReAssign(\'' + $valuationId + '\',\'' + $user + '\')">Re-Assign</a>';
                          #  echo '<a href="" onclick="upBanner(\''+$row['banner_id']+'\',\''+0+'\')" class="btn btn-default btn-xs"><i class="fa fa-arrow-up"></i></a>';}
                        if($i==mysql_num_rows($data)){

                        }else{
                           // echo $i."and".mysql_num_rows($data)."";
               # echo '<a href="" onclick="upBanner(\''+$row['banner_id']+'\',\''+1+'\')" class="btn btn-default btn-xs"><i class="fa fa-arrow-down"></i></a>';}
               echo "<a href='' onclick='downBanner(".$row['sort_order'].")' class='btn btn-default btn-xs'><i class='fa fa-arrow-down'></i></a>";}
							echo "
													
								<a href='form_banner.php?ban_id=".$row['banner_id']."' class='btn btn-info btn-xs'><i class='fa fa-pencil-square-o'></i> แก้ไข</a>
								";
								if($row['active'] == 0){
									echo " <a href='#' class='btn btn-success btn-xs' onclick='cmdShow(".$row['banner_id'].")'><i class='fa fa-minus-square-o'></i> แสดง</a>";
								}else{
									echo " <a href='#' class='btn btn-warning btn-xs' onclick='cmdHidden(".$row['banner_id'].")'><i class='fa fa-minus-square-o'></i> ซ่อน</a>";
								}
														
									echo " <a href='#' class='btn btn-danger btn-xs' onclick='cmdDel(".$row['banner_id'].")'><i class='fa fa-trash-o'></i> ลบ</a>
					...
					echo "	<div class='km_box_2'>
							...
							<div class='col-md-7 col-sm-7'>
								<h4>".+$i ." <span class='th'>".$row['title']."</span> <span class='en'>".$row['title_en']."</span></h4>
								<p>".$row['lastDate']."</p>
...
function upBanner(order){
    ...
    $.ajax({
        ...
        data:  { order : order, type : 0, table : "t_banner" },
        success: function(sxAjax){
            if (sxAjax.substr(0,9) == "Completed") {
...
function downBanner(order){
    ...
    $.ajax({
        type: "POST",
        url: "function/updatesortorder.php",
        data:  { order : order, type : 1, table : "t_banner" },
        success: function(sxAjax){
            if (sxAjax.substr(0,9) == "Completed") {
