		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
						...
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_freq.php?freq_id=$freq_id");
		            }
		        }
		    }
		    ...
		    if ($_FILES['files']['error'][$f] == 0) {	           
		        if ($_FILES['files']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["files"]["tmp_name"][$f], $mpath.$name)) {
		            	$sql = "INSERT INTO t_freq_file
							(freq_id,name,path,create_id,create_date,update_id,update_date,active)
							VALUES
							('$freq_id','$name','$path$name','$userid',now(),'$userid',now(),'1')
							";
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            }
...
<div class="container">
		<div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">คำถามที่พบบ่อย</h3>
				<ol class="breadcrumb">
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
                        ...
                        <div class="panel-body">
						   ...
						   <input type="hidden" name="freq_id" id="freq_id" value="<?=$_GET['freq_id'] ?>">
...
function cmdSave(){
	...
	params = params + "&" + $('#freq_id').serialize();
