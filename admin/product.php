		...
		<div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">
					<span class="en"> สินค้าวิสาหกิจชุมชน</span>
					<span class="th"> สินค้าวิสาหกิจชุมชน</span>
				...
				$sql = "SELECT * FROM t_product ";
					...
					echo "<div  class='adminDiv text-right'> ";
							echo "
								...
								<a href='form_product.php?product_id=".$row['product_id']."' class='btn btn-info btn-xs'><i class='fa fa-pencil-square-o'></i> แก้ไข</a>
								";
								if($row['active'] == 0){
									echo " <a href='#' class='btn btn-success btn-xs' onclick='cmdShow(".$row['product_id'].")'><i class='fa fa-minus-square-o'></i> แสดง</a>";
								}else{
									echo " <a href='#' class='btn btn-warning btn-xs' onclick='cmdHidden(".$row['product_id'].")'><i class='fa fa-minus-square-o'></i> ซ่อน</a>";
								}
														
									echo " <a href='#' class='btn btn-danger btn-xs' onclick='cmdDel(".$row['product_id'].")'><i class='fa fa-trash-o'></i> ลบ</a>
					...
					echo "	<div class='km_box_2'>
							...
							<div class='col-md-7 col-sm-7'>
								...
								<a href='".$row['url']."' >แหล่งที่มา</a> 
								<p>".$row['update_date']."</p>
								<a href='product_detail.php?Id=".$row['product_id']."' class='btn btn-default'>ข้อมูลเพิ่มเติม</a> 
					";
					if($row['path'] == ""){
						echo "";
...
function cmdNew(){
	...
	$.ajax({
		type: "POST",
		url: "function/add_product.php",
		data: params,
		success: function(sxAjax){			
			if (sxAjax.substr(0,9) == "Completed") {
                document.location.href="form_product.php?"+"product_id=" + (sxAjax.substr(10)) ;
