		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
						...
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_event.php?event_id=$event_id");
		            }
		        }
		    }
		    ...
		    if ($_FILES['files']['error'][$f] == 0) {	           
		        if ($_FILES['files']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["files"]["tmp_name"][$f], $mpath.$name)) {
		            	$sql = "INSERT INTO t_event_file
							(event_id,name,path,create_id,create_date,update_id,update_date,active)
							VALUES
							('$event_id','$name','$path$name','$userid',now(),'$userid',now(),'1')
							";
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            }
...
<div class="container">
		<div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">ปฏิทินกิจกรรม</h3>
				<ol class="breadcrumb">
								...
								<div class="form-group">
									<div class="col-md-6">
										<div class="form-group">
											...
											<input class="form-control" type="text" id="startDate" name="startDate" value="<?=$row['start'] ?>" />
									...
									<div class="col-md-6">
										<div class="form-group">
										<label>วันที่สื้นสุด</label>
										<input class="form-control" type="text" id="endDate" name="endDate" value="<?=$row['end'] ?>"/>
										<p class="help-block">โปรดระบุ</p>
										</div>
									</div>
								</div>
...
function cmdSave(){
	...
	params = params + "&" + $('#event_id').serialize();
	params = params + "&" + $('#startDate').serialize();
	params = params + "&" + $('#endDate').serialize();
