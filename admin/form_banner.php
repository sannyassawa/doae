		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
		            	...
		            	header("Refresh:0; url=form_banner.php?ban_id=$banner_id");
		            }
		        }
		    }
		    ...
		    if ($_FILES['files']['error'][$f] == 0) {	           
		        if ($_FILES['files']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["files"]["tmp_name"][$f], $mpath.$name)) {
		            	$sql = "INSERT INTO t_banner_file
							(banner_id,name,path,create_id,create_date,update_id,update_date,active)
							VALUES
							('$banner_id','$name','$path$name','$userid',now(),'$userid',now(),'1')
							";
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            }
...
<div class="container">
		<div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">จัดการแบนเนอร์</h3>
				<ol class="breadcrumb">
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
                        ...
                        <div class="panel-body">
						   ...
						   <input type="hidden" name="banner_id" id="banner_id" value="<?=$_GET['ban_id'] ?>">
						   <input type="hidden" name="user_id" id="user_id" value="<?=$_SESSION['userid'] ?>">
								<div class="form-group">
                                    <label>รูปภาพหน้าปก</label>
                                    <p><img src="../<?=$row['images'] ?>" height="400" width="100%" /></p>
									...
									<p class="help-block">โปรดระบุ ขนาดรูปภาพ : 1024 x 450px</p>
                                </div>
...
function cmdSave(){
	...
	params = params + "&" + $('#banner_id').serialize();
