...
$cat_news_id = $_GET['cat_news_id'];
	...
	$sql = "select * from t_news_cat
			where cat_news_id = ".$_GET['cat_news_id'];
		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
		            	$sql = "Update t_news_cat set
								icon = '$path_img$name'
								where cat_news_id = $cat_news_id ";
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_news_cat.php?cat_news_id=$cat_news_id");
		            }
		        }
		    }
...
<div class="container">
		<div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">จัดการหมวดหมู่ข่าว</h3>
				<ol class="breadcrumb">
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
                        ...
                        <div class="panel-body">
						   ...
						   <input type="hidden" name="cat_news_id" id="cat_news_id" value="<?=$_GET['cat_news_id'] ?>">
...
function cmdCancel(){
	...
	window.location.href = "news_catergory.php";
...
function cmdSave(){
	...
	$.ajax({
		type: "POST",
		url: "function/save_cat_news.php",
		data: params,
