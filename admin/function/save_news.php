	...
	if (mysql_query("BEGIN",$conn)) {
			...
			if($news_id == 0){
				$sql = " INSERT ignore INTO t_news
					 (cat_news_id,title_en,title,content_en,content,create_id,create_date,update_id,update_date,active,flag_index) ";
				$sql .= " VALUES ";
				$sql .= "( $cat_news_id,$title_en,$title,$content_en,$content,$userid,now(),$userid,now(),1,$flag_index )";
