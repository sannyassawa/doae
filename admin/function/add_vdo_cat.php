<?php
	...
	if (mysql_query("BEGIN",$conn)) {
			...
			$sql = " SELECT MAX(cat_vdo_id) AS mns";
			$sql .= " FROM t_vdo_cat  ";
			...
			$sql = " INSERT ignore INTO t_vdo_cat
					 ( create_id, create_date, update_id,update_date,active) ";
			$sql .= " VALUES ";
			$sql .= "( $userid, now(), $userid,now(),1 )";
			...
			$sql = " SELECT MAX(cat_vdo_id) AS MaxID ";
			$sql .= " FROM t_vdo_cat  ";
