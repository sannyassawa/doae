...
$count = 0;
$cat_article_id = $_GET['cat_article_id'];
	...
	$sql = "select * from t_article_cat
			where cat_article_id = ".$_GET['cat_article_id'];
		    ...
		    if ($_FILES['images']['error'][$f] == 0) {	           
		        if ($_FILES['images']['size'][$f] > $max_file_size) {
		            ...
		            if(move_uploaded_file($_FILES["images"]["tmp_name"][$f], $mpath_img.$name)) {
		            	$sql = "Update t_article_cat set
								icon = '$path_img$name'
								where cat_article_id = $cat_article_id ";
						mysql_query($sql);
		            	$count++; // Number of successfully uploaded files
		            	header("Refresh:0; url=form_article_cat.php?cat_article_id=$cat_article_id");
		            }
		        }
		    }
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
                        ...
                        <div class="panel-body">
						   ...
						   <input type="hidden" name="cat_article_id" id="cat_article_id" value="<?=$_GET['cat_article_id'] ?>">
						   <input type="hidden" name="user_id" id="user_id" value="<?=$_SESSION['userid'] ?>">
						   
						   <div class="col-md-4">
								<div class="form-group">
                                    <label>ICON</label>
                                    <p><img src="../<?=$row['icon'] ?>" height="200" width="200" /></p>
									...
									<p class="help-block">โปรดระบุ ขนาดรูปภาพ : 200 x 200px</p>
                                </div>
						   </div>
						   <div class="col-md-8">
								<div class="form-group">
...
function cmdSave(){
	...
	$.ajax({
		type: "POST",
		url: "function/save_cat_article.php",
		data: params,
